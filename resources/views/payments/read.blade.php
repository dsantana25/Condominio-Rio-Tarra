...
<div class="container" id="condominio">
  <div class="row">
    <div class="col-lg-6 col-sm-12 mx-auto">
      ...
      <div class="card border">
        ...
        <div class="card-body bg-light" style="height:450px">
          <transition :duration="{ enter: 1000, leave: 1 }" name="custom-classes-transition" enter-active-class="animated fadeInLeft" leave-active-class="animated fadeInLeft">
            <template v-if="pagination.currentPage == 1">
              <div class="row text-center">
                ...
                <div class="col-12">
                  <p class="text-lg-left text-sm-center" style="font-weight:600">Seleccionar apartamento</p>
                  <div class="list-options row">
                    <label class="apartment-icon col" v-bind:class="{'selected': payment.apartment.floor == 'PB' }">
                      <input id="radio1" value="PB" v-model="payment.apartment.floor" name="radio" type="radio" class="apartment-icon-input">
                      <span>PB</span>
                  ...
                  <template v-if="payment.apartment.floor != ''">
                    <div class="my-4">
                      <h1 class="new-title text-center">@{{payment.apartment.floor}}-@{{payment.apartment.side}}</h1>
                    </div>
                  </template>
                  <div class="my-4" v-else>
                    <h1 class="new-title text-center">-</h1>
                  </div>
                </div>
                <div class="col-12 mx-auto">
                  <button @click="getApartmentData()" v-bind:class="{'disabled': !checkApartmentSelected()}" class="btn btn-primary" type="button" name="button"><i class="material-icons align-middle">search</i>Buscar</button>
                </div>
              </div>
          ...
          <transition :duration="{ enter: 1000, leave: 1 }" name="custom-classes-transition" enter-active-class="animated fadeInLeft" leave-active-class="animated fadeInLeft">
            <template v-if="pagination.currentPage == 2">
              <div class="apartmentData">
                <h4 class="new-title text-center">@{{payment.apartment.floor}}-@{{payment.apartment.side}}</h4>
                <p class="owner-info">@{{apartment.name}}</p>
                <p class="owner-info mb-5">C.I @{{apartment.identity}}</p>
                <p class="owner-info">Ultimo pago:</p>
                <p v-if="apartment.lastMonthPaid != 0" class="owner-info">@{{apartment.lastMonthPaid}}</p>
                <p v-else class="owner-info text-danger">No hay pagos registrados</p>
                <button @click="nextPage()" class="btn btn-primary btn-block mt-4" type="button" name="button"><i class="material-icons align-middle mr-4">money</i>Pagar condominio</button>
              </div>
            </template>
          </transition>
          <transition :duration="{ enter: 1000, leave: 1 }" name="custom-classes-transition" enter-active-class="animated fadeInLeft" leave-active-class="animated fadeInLeft">
            <template v-if="pagination.currentPage == 3">
              <div class="apartmentData row h-100">
                <div class="col-12">

                  <div class="my-form-group">
                    <p class="text-center" style="font-weight:600">Ultimo pago:</p>
                    <p v-if="apartment.lastMonthPaid != 0" class="owner-info">@{{apartment.lastMonthPaid}}</p>
                    <p v-else class="owner-info text-danger">No hay pagos registrados</p>
                  </div>

                  <div class="my-form-group">
                    <p class="text-center" style="font-weight:600">Meses a pagar:</p>
                    <div class="button-group text-center">
                      <button @click="substractMonth" class="btn btn-primary rounded-circle p-1" type="button"><i class="material-icons align-middle" style="font-size:25px">remove</i></button>
                      <span class="owner-info mx-5">@{{payment.months.value}}</span>
                      <button @click="addMonth" class="btn btn-primary rounded-circle p-1" type="button"><i class="material-icons align-middle" style="font-size:25px">add</i></button>
                    </div>
                  </div>

                  <p class="text-center" style="font-weight:600">Tipo de pago:</p>

                  <div class="row justify-content-center text-center">
                    <label class="payment_type col-auto" v-bind:class="{'selected': payment.paymentType == '1' }">
                      <input id="radio1" value="1" v-model="payment.paymentType" name="radio" type="radio" class="apartment-icon-input">
                      <i class="material-icons d-block">attach_money</i>Efectivo
                    </label>
                    <label class="payment_type col-auto" v-bind:class="{'selected': payment.paymentType == '2' }">
                      <input id="radio1" value="2" v-model="payment.paymentType" name="radio" type="radio" class="apartment-icon-input">
                      <i class="material-icons d-block">attach_money</i>Depósito
                    </label>
                    <label class="payment_type col-auto" v-bind:class="{'selected': payment.paymentType == '3' }">
                      <input id="radio1" value="3" v-model="payment.paymentType" name="radio" type="radio" class="apartment-icon-input">
                      <i class="material-icons d-block">attach_money</i>Transferencia
                    </label>
                    <label class="payment_type col-auto" v-bind:class="{'selected': payment.paymentType == '4' }">
                      <input id="radio1" value="4" v-model="payment.paymentType" name="radio" type="radio" class="apartment-icon-input">
                      <i class="material-icons d-block">attach_money</i>Cheque
                    </label>
                  </div>
                  ...
                  <div style="position: absolute;width: 100%;left: 0;padding-left: 0%;padding: 0 20px;top: 90%;">
                    <button @click="savePayment" type="button" class="btn btn-primary btn-block" style="position: relative;margin: 0 auto;" name="button">Registrar pago</button>
                    {{-- <button @click="toggle.newPayment = false" type="button" class="btn btn-danger btn-block" name="button">Cancelar</button> --}}
                  </div>
              </div>
            </template>
